...
- Helper exit 11 (`AMBIGUOUS_DEVICE`) maps to a general MCP message about an unambiguous target. Capture also uses that code when no UDID is configured, not only when several Apple TVs are visible. The helper's own stderr still names the specific case. MCP does not parse stderr.
- When every AUTO transport fails, the helper reports the userspace outcome if userspace ran. Native pairing and userspace pairing are different credential domains.
